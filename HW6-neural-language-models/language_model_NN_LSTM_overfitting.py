...
learning_rate = 0.01#0.004 #0.02 -- blow up (or overfitting)
# 0.0001
...
#     output_real = tag_scores[-1].view(1,-1)
    ...
    loss.backward()
    learning_rate_ad = learning_rate * np.sqrt(max(pre_err_rate - num_err*1.0/iter,0))*5
    for p in lstm.parameters():   p.data.add_(-learning_rate_ad, p.grad.data)
#     optimizer.step()
    optimizer = optim.SGD(lstm.parameters(), lr=learning_rate_ad)
...
def predict(input_line, n_predictions=3):
    ...
    output = evaluate(Variable(trigramTensor(input_line)))

...
lower  = False
zeros = False 
test_sentences = loader.load_sentences(opts.input, lower = lower,zeros = zeros)
test_data = loader.prepare_dataset(test_sentences, word_to_id, char_to_id, tag_to_id, lower = lower)
# print("test_sentences",test_sentences)

with codecs.open(opts.input, 'r', 'utf-8') as f_input: # this is not used
    count = 0
    # for line in f_input:
    ...
    for raw_sentence, data in zip(test_sentences, test_data):
        ...
        words =[line[0] for line in raw_sentence]
        # Prepare input
